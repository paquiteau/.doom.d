...
(after! org
  (setq
   ...
   org-cite-global-bibliography '("/home/pac/org/library.bib")
...
tab-indent."
  ...
  (or (copilot-accept-completion)
      (company-indent-or-complet-common nil)))
...
(defvar rk/no-copilot-buffers
  '("library.bib"
    "\\*.*?\\*")
    "Buffer in which copilot is not required")
...
(defun string-matches-any-regex (string regex-list)
  "Check if STRING matches any regular expression in REGEX-LIST."
  (catch 'matched
...
(defun rk/copilot-disable-predicate ()
  ...
  (or rk/copilot-manual-mode
      ...
      (string-matches-any-regex buffer-name rk/no-copilot-buffers)
      (+popup-buffer-p)
...
(defcustom lsp-pylsp-plugins-mypy-enabled nil
  ...
  (lsp-register-custom-settings '(("pylsp.plugins.ruff.enabled" lsp-pylsp-plugins-ruff-enabled  nil)))
...
#+end_src
* COMMENT LaTeX OCR
Using rapid_latex_ocr (a faster alternative to pix2tex, which is a free alternative to mathpix)

=pix2tex= command is installed in a virtualenv of the same name.
#+begin_src bash :tangle ~/bin/latex_ocr.sh :tangle-mode (identity #o755) :results output
#! /usr/bin/bash

MODEL_DIR="$HOME/.cache/rapid_latex_ocr/models"
IMG_FILE="$HOME/.cache/rapid_latex_ocr/tmp_img.png"
gnome-screenshot -a -f $IMG_FILE

PYENV_VERSION=pix2tex rapid_latex_ocr $IMG_FILE \
    --image_resizer_path $MODEL_DIR/image_resizer.onnx\
    --encoder_path $MODEL_DIR/encoder.onnx \
    --decoder_path $MODEL_DIR/decoder.onnx \
    --tokenizer $MODEL_DIR/tokenizer.json | sed 's/\\\\/\\/g'
#+end_src

Then it is just a matter

#+begin_src emacs-lisp
(setq latex-ocr-cmd "~/bin/latex_ocr.sh")
(defun pac/latex-ocr-insert (mathdelim)
  (interactive)
  (let ((raw_output (shell-command-to-string latex-ocr-cmd)))
    (insert mathdelim)
    (backward-char 2)
    (insert (replace-regexp-in-string "'\\(.*?\\)', [0-9].[0-9]+)\n?" "\\1" raw_output))
    (forward-char 2)
))
...
\(h_{\sigma}:x\rightarrow\frac{1}{2}\,\vert\vert x\vert\vert^{2}-g_{\sigma}(x),\)
