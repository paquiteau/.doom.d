...
;; Use tecosaur development branch
 (package! org :recipe
   ...
   :pin "b70c0cc9667415860211d37d3eb79fe823f1e723")
#+end_src
** Global config
...
#+end_src
#+begin_src emacs-lisp
(setq pac/llamafile-server-port "9090")
(setq pac/llamafile-cmd "Meta-Llama-3-8B-Instruct.Q5_K_M.llamafile")

(use-package! gptel

  :config
  (setq gptel-model "llamafile")
  (setq gptel-backend (gptel-make-openai "llamafile"
                        :stream t
                        :protocol "http"
                        :host (format "localhost:%s" pac/llamafile-server-port)
                        :models '("llamafile"))

        ))

;; written by Llama3
(defun my-llama-launch ()
  "Launch a llamafile in a subprocess."
  (interactive)

  (let* ((process-name pac/llamafile-cmd)
         (buffer-name "*llamafile-srv*")
         (args (list "--server" "--port" pac/llamafile-server-port "--nobrowser" "-ngl" "99" ))  ;; Replace with the appropriate arguments for your server
         (buffer (get-buffer-create buffer-name))
         (process (apply 'start-process pac/llamafile-cmd buffer pac/llamafile-cmd  args)))
    ;; Set up the process sentinel
    (message "Started  %s ." process-name)
    (set-process-sentinel
     process
     (lambda (proc event)
       (when doom-debug-mode
         (message "Process %s received event %s" (process-name proc) event))
       (cond
        ((string-match "finished" event)
         (when init-file-debug
           (message "Server process finished successfully.")))
        ((string-match "exited abnormally" event)
         (when init-file-debug
           (message "Server process exited abnormally."))))))
    ;; Optionally, set up a process filter if needed
    (set-process-filter
     process
     (lambda (proc output)
       (when init-file-debug
         (with-current-buffer (process-buffer proc)
           (insert output)
           (message "Process %s output: %s" (process-name proc) output)))))))
;; Usage: M-x launch-server-process
  ...
  (let* ((process-name pac/llamafile-cmd)
    ...
    (if process
        (progn
          ...
          (message "Server process %s stopped." process-name))
      (message "No server process %s is running." process-name))))

;; Usage: M-x stop-server-process
...
(after! lsp-pylsp
  (setq lsp-pylsp-plugins-pylint-enabled nil
        ...
        lsp-pylsp-plugins-mypy-overrides ["--ignore-missing-imports" t]
        )
