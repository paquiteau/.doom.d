...
(defun pac/load-catppuccin (flavor)
  (let ((cursor-color (catppuccin-color 'rosewater flavor)))
    (evil-set-cursor-color cursor-color)
    (put 'cursor 'evil-normal-color cursor-color))
    (catppuccin-load-flavor flavor))
(defun pac/load-catppuccin-mocha ()
  (pac/load-catppuccin 'mocha))

(defun pac/load-modus-operandi ()
  (let ((cursor-color "#000000"))
    (evil-set-cursor-color cursor-color)
    (put 'cursor 'evil-normal-color cursor-color))
    (load-theme 'modus-operandi))

(defun pac/load-doom-dracula ()
  (let ((cursor-color "#bd93f9"))
    (evil-set-cursor-color cursor-color)
    (put 'cursor 'evil-normal-color cursor-color))
    (load-theme 'modus-operandi))
...
(use-package! auto-dark
  ...
  (setq! custom-safe-themes t) ;; all themes are safe to load
  (add-hook 'auto-dark-light-mode-hook #'pac/load-modus-operandi)
  (add-hook 'auto-dark-dark-mode-hook #'pac/load-catppuccin-mocha)
...
(setq
 ...
 doom-big-font (font-spec :family "Iosevka Nerd Font" :size 18.0)
 doom-serif-font (font-spec :family "Iosevka Nerd Font" :size 12.0)
...
(defun pac/insert-before (lst new-elt sym)
  "Insert NEW-ELT before SYM in LST by mutating the list.
Does nothing if SYM is not found or is the first element."
  (let ((cur lst))
    (while (and (cdr cur) (not (eq (cadr cur) sym)))
      (setq cur (cdr cur)))
    (when (and (cdr cur) (eq (cadr cur) sym))
      (setcdr cur (cons new-elt (cdr cur)))))
  lst)
#+end_src
...
;;    (when (and (eq major-mode 'org-mode) (org-in-subtree-not-table-p) (not (org-in-src-block-p)) (not (texmathp)))
    (mark-paragraph)
    ;; First remove line wrap that cuts wor-
    ;; ds in two
...
#+end_src
** Buffer Managements
*** Burry Compilation buffer when success
#+begin_src emacs-lisp
...
(with-eval-after-load 'flycheck-grammary
...
#+end_src
*** Ignored files in status buffer
Getting a potential view of ignored files can be helpfull as well
per [[https://emacs.stackexchange.com/a/28506]]
#+begin_src emacs-lisp
...
(with-eval-after-load 'magit
  (pac/insert-before magit-status-sections-hook #'magit-insert-ignored-files #'magit-insert-untracked-files)
  )
#+end_src
...
** Marginalia
Quite good ootb, but there is some things from Teco:
#+begin_src emacs-lisp
...
(add-hook 'pdf-view-mode-hook #'(lambda ()
                                  (ultra-scroll-mode -1) ;; disable ultra-scroll to avoid conflict on mapping scroll wheel
...
** Typst
Is this the new LaTeX ? Let's find out
#+begin_src emacs-lisp :tangle "packages.el"
;; in packages.el
...
(defun pac/typst-start-slides ()
  (interactive)
  (typst-preview-mode 0) ;; Disable
  (revert-buffer) ;; Make sure we are syncd.
  (let ((typst-preview-preview-mode "slide")
        (typst-preview--master-file (buffer-file-name))
        )
    (typst-preview-start)
    (typst-preview-open-browser)
    ))
...
(with-eval-after-load 'citar
  ...
  ;; use external pdf reader for now
  (cl-pushnew '("pdf" . citar-file-open-external) citar-file-open-functions)
...
Note that there is also =org-babel-demarcate-block=, which only works in src-block, and break stuff if call with point in the middle of the line.
#+begin_src emacs-lisp
...
Make the call to split block easy with =M-RET= binding.
#+begin_src emacs-lisp
...
\addtokomafont{title}{\let\huge\large} % best way to change the title size really ?
\setkomafont{author}{\normalfont\small}
\setkomafont{date}{\normalfont\small}
...
(with-eval-after-load 'ox-latex
  (let* ((article-sections '(("\\section{%s}" . "\\section*{%s}")
    ...
    (setcdr (assoc "article" org-latex-classes)
            `(,(concat "\\documentclass[a4paper, 12pt, DIV=16]{scrartcl}" hanging-secnum-preamble)
              ,@article-sections))
...
I use the roam dailies as a very crude and *flat*  notebook, each entry is under a top level headline, with a timestamp and then I use a custom title, tags, handles, etc...
...
(with-eval-after-load 'org-roam
  ...
  ;; Disable spurious completion in math.
    (defadvice! org-roam-complete-everywhere-but-math (orig-fun) :around #'org-roam-complete-everywhere
      (unless (org-inside-LaTeX-fragment-p) (funcall orig-fun)))
  )
#+end_src
#+begin_src emacs-lisp
...
#+end_src
*** Journal Daily
The journal consist of a list of top level heading that look like this  =*<2000-01-01 Wed>XXXX= 
#+begin_src emacs-lisp
(with-eval-after-load 'org-roam
  (setq 
        ...
        '(("d" "Daily Notes" plain
           "%?"
           :target (file+olp "journal.org" ("<%<%Y-%m-%d %A>>"))
          ))))
#+end_src
Let's make accessing the journal more easily
Bind to ~SPC j~ *Disabled as we used for SPC J for GTD*
#+begin_src emacs-lisp
;;(map! :leader :desc "Open Today log" "j" #'org-roam-dailies-goto-today)
#+end_src

Exclude the journal from the roam database
#+begin_src emacs-lisp
...
** My Org-GTD
:PROPERTIES:
:PROJECT_ID: my-org-gtd
:END:
I have try to commit to getting organize more and more, after some testing with [[https://github.com/trevoke/org-gtd.el][org-gtd.el]], I have decided to start from first principles again. Unfortunatly, this mean RTFM, find other setups, and remove configuration from the raw org config as well.
#+begin_src emacs-lisp
;;
(setq org-todo-keywords '((sequence
      ...
      org-modern-todo-faces '(
                              ("TODO" . (:background "darkred" :weight bold))
                              ("NEXT" . (:background "green" :weight bold))
                              ("WAIT" . (:background "darkorange" :weight bold))
                              )
      org-agenda-files '("~/org/gtd")
      org-refile-targets '(("~/org/gtd/next.org"  . (:level  . 0))
...
(setq org-capture-templates
      `(("t" "Task" entry  (file "~/org/gtd/inbox.org")
         ,(string-join '("* TODO %?"
                         ":PROPERTIES:"
                         ":CREATED: %U"
                         ":END:"
        ...
        ("i" "infos" entry (file "~/org/gtd/inbox.org")
         ,(string-join '("* %?"
                         ":PROPERTIES:"
                         ":CREATED: %U"
                         ":END:"
        ...
        ("p" "project" entry (file "~/org/gtd/projects.org") 
         ,(string-join '("* %? ^g" 
                         ":PROPERTIES:"
                         ":CREATED: %U"
                         ":END:") "\n")
         :before-finalize (,(lambda () (org-set-property "PROJECT_ID"  (pac/slugify (org-get-heading t t t t)))) 
         )
        )))
...
#+end_src
*** Refile DONE state to org-roam dailies
...
;; remove all sub-refile command for now,
;; Ideally we want to setup a transient interface for the refile, à-là org-gtd 
(map! :map org-mode-map :localleader "r" #'org-refile)
(map! :map doom-leader-map "o a" #'org-agenda)
...
(after! org-super-agenda
  (map! :map org-super-agenda-mode-map
...
(setq org-agenda-skip-scheduled-if-done t
      ...
      org-agenda-start-day "-3d" ;; see 3days before the current day
      org-agenda-deadline-faces '((1.0 . org-imminent-deadline) (0.5 . org-upcoming-deadline)
...
(defun pac/format-prefix-project ()
  "Format the prefix to be the name of the project (parent header)"
  ;; TODO use a PROJECT_ID 
  (format "%-20s" (or (car (last (org-get-outline-path))) ""))
  )
(defun pac/header-face (s)
  "Customize faces of the header string for the agenda"
  (add-face-text-property 0 (length s) '(:weight bold :height 2) nil s)
  s
  )
(set-face-attribute 'org-agenda-structure nil :weight 'bold :height 1.1)
;; Customizing agenda
(setq org-agenda-time-grid '((daily) (800 1200 1800) "---" "-----"))
         ...
         (
          ...
          (todo "NEXT"
                ((org-agenda-skip-function
                 ...
                 (org-agenda-overriding-header (pac/header-face "🚀 Just Do It"))
                 (org-agenda-prefix-format "  %(pac/format-prefix-project) %s")
                 ))
          (agenda ""
          ...
          (tags "inbox"
                ((org-agenda-prefix-format "  %?-12t% s")
                 (org-agenda-hide-tags-regexp "inbox") ;; we are already filtering
                 (org-agenda-overriding-header "📥 Inbox: Clarify and Organize")))

          (tags-todo "-inbox"
                     ((org-agenda-skip-function
                       '(org-agenda-skip-entry-if 'deadline 'scheduled))
                     (org-agenda-overriding-header "🗂️ Projects")
                      (org-agenda-prefix-format "  %(pac/format-prefix-project) %s")
                      ))
          (tags "CLOSED>=\"<today>\""
                ((org-agenda-overriding-header "✔️ Completed today")))))
        ("c" "Super view" 
...
(defun pac/TeX-command-current-frame ()
 "Find the outermost enclosing LaTeX 'frame' environment and execute `TeX-command-region` on it."
  (interactive)
  (save-excursion
    (let ((found nil))
      (while (and (not found) (LaTeX-current-environment))
        (if (string= (LaTeX-current-environment) "frame")
            (setq found t)
          (ignore-errors (LaTeX-find-matching-begin))))
      (if found
          (let ((begin (save-excursion (LaTeX-find-matching-begin) (point)))
            ...
            (push-mark end t t)
            (TeX-command-region))
        (message "No enclosing 'frame' environment found.")))))
...
(setq TeX-fold-math-spec-list
      `(;; missing/better symbols
        ...
        (,(lambda (num den)
                   (if
                       (and (TeX-string-single-token-p num) (TeX-string-single-token-p den)) (concat num "／" den)
                 (concat "❪" num "／" den "❫"))) ("frac"))
...
(with-eval-after-load 'laas
  ...
  (aas-set-snippets 'laas-mode
                    ...
                    "ej" (lambda () (interactive) (yas-expand-snippet "e^{j $1} $0"))
                    "ei2p" (lambda () (interactive) (yas-expand-snippet "e^{2\\imath\\pi $1} $0"))
                    "dp" (lambda () (interactive) (yas-expand-snippet "\\frac{\\partial $1}{\\partial $2} $0"))
                    "dd" (lambda () (interactive) (yas-expand-snippet "\\frac{d $1}{d $2} $0"))
                    "j2p" "j2\\pi"
                    "CC" "\\mathbb{C}"
                    "CN" "\\mathbb{C}^N"
                    "EE" "\\mathbb{E}"
...
(use-package! copilot-chat
  :after (org markdown-mode request))
#+end_src
** Editor-Code Assistant (ECA) :testing:
...
(require 'agent-shell)
#+end_src
Setup connection to LLAMA CPP
#+begin_src json :tangle-mode (identity #o444) :tangle "~/.config/eca/config.json" :mkdirp t
{
    "providers": {
        "llamacpp": {
        "api": "openai-chat",
        "fetchModels": true,
        "url": "http://10.10.10.3:8080/v1",
        models":{
        "Qwen3.6": {},
        }
    }
...
Ideally, use ruff format everywhere, unless the project has black configured (ie check the toml file).
#+begin_src emacs-lisp
(add-hook 'python-mode-hook #'(setq-local fill-column 88)) ;; TODO get this from the toml file if it exists.
#+end_src

Managing virtualenv is somewhat a pain..
#+begin_src emacs-lisp
(use-package! pet
  :config
  (add-hook 'python-base-mode-hook
            (lambda ()
              ...
              (pet-dape-setup))))
#+end_src
Code cell for vs-code like experience 
#+begin_src emacs-lisp
(use-package! code-cells)
(add-hook 'python-mode-hook 'code-cells-mode-maybe)
#+end_src
** Multiplexed LSP setup
using eglot and rassumfrassum, merge the output from =ty= (typechecker) and =ruff= (linter/formatter)
#+begin_src emacs-lisp :tangle "packages.el"
                                 ...
                                 (let ((python (dape-config-get config 'command)))
                                   (unless
                                     ...
                                     (user-error "%s module debugpy is not installed"
                                                 python))))
                        command (pet-executable-find "python")
...
#+begin_src emacs-lisp
;; Add a pytest-debug-this test
(defun pytest-dape-this-test ()
  (interactive)
  (dape `(command ,(pet-executable-find "python")
          command-args ("-m" "debugpy.adapter" "--host" "0.0.0.0" "--port" :autoport)
          port :autoport
          :request "launch"
          :type "python"
          ...
          :cwd ,(dape-cwd)
          :module "pytest"
          :args ["-k" ,(python-pytest--node-id-def-at-point-treesit)]
          :justMyCode nil
          :console "integratedTerminal"
          :showReturnValue t
          :stopOnEntry nil)))
...
#+begin_src emacs-lisp
;;(with-eval-after-load 'lsp-mode (setq lsp-diagnostic-provider :flymake))
#+end_src
** Copilot
#+begin_src emacs-lisp
  ...
  :config
    (add-to-list 'eglot-server-programs `(
      (python-mode python-ts-mode)
         ,(expand-file-name "~/.local/bin/ty") "server"
    ))
    ;; (add-to-list 'eglot-server-programs '(
    ;;   (python-mode python-ts-mode)
...
(use-package! eglot-ltex-plus
  :init
  (setq eglot-ltex-plus-server-path (expand-file-name "~/.config/emacs/.local/etc/lsp/ltex-ls-plus-18.6.1"))
...
#+begin_src bash :tangle "~/bin/latex_ocr.sh" :tangle-mode (identity #o775)
#!/bin/bash
# ~/bin/latex_ocr.sh
