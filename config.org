...
(defvar fancy-splash-image-template
  (expand-file-name "misc/splash/emacs-e-template.svg" doom-private-dir)
...
(package! org-ref
  ...
  :pin "37b64e6cc1068c1b7ffe579851a345aa57772333")
(package! citeproc)
...
#+end_src
Speaking of headlines, a nice package
** Custom functions
#+begin_src emacs-lisp :tangle yes
...
#+end_src
** Treemacs dired
To have color icons in dired buffer. It only works in graphical session.
#+begin_src emacs-lisp
...
(after! consult
  ...
  (setf (plist-get (alist-get 'perl consult-async-split-style-alist) :initial)
        ";"))
#+end_src
** Deft
...
\\newenvironment{proof}[1][]{{\\par \\noindent\\textit{{\\proofname#1}}: \\par}}{\\hfill$\\blacksquare$}
...
(map! :leader
      :prefix-map ("n" . "notes")
       :desc "Bibliographic entries"  "b" #'helm-bibtex ;; helm is clearly superior here
      (:prefix ("r" . "roam ")
       :desc "Insert node Immediatly" "I" #'org-roam-node-insert-immediate
       :desc "Insert screenshot" "c" #'pac/screenshot-to-roam))
#+end_src
** Bibliography
I use Zotero and the BetterBibTex plugins to gather all the references. It keep up-to-date a biblatex file.
The MVP here is org-roam-bibtex which connect the bib search interface ~helm-bibtex~ and ~org-roam~ it also tweak ~org-ref~ to behave nicely. In particular, there is no need to configure the =bibtex-completion-notes= file template, as we are going to use the one provided by org-roam.

#+begin_src emacs-lisp
(defvar pac-bibliography "~/org/library.bib")
(use-package! bibtex-completion
  :config
  (setq bibtex-completion-bibliography pac-bibliography
        bibtex-completion-notes-path "~/org/roam/bib/"
...
(use-package! org-roam-bibtex
  ...
  (add-to-list
               ...
               '("r" "Bib Ref" plain (file "~/org/roam/templates/bib.org") :target
                 (file+head "bib/${citekey}.org" "#+title:${citekey}")
                 :unnarrowed t)
               t)

  )

;; Alternatively, call `org-roam-bibtex-mode' interactively
(org-roam-bibtex-mode)

#+end_src

Lets take some notes on pdf with org noter
#+begin_src emacs-lisp
