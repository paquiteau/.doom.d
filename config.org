...
(setq
 ...
 projectile-ignored-projects '("~/" "/tmp" "~/.emacs.d/.local/straight/repos/")
 projectile-project-search-path '(("~/gits/nsp/" . 1) ("~/gits/rep/" . 1))
...
#+begin_src emacs-lisp
(setq conventional-commit-type  '(("feat" . " A new feature")
                                  ("fix" . " A bug fix")
...
(defun pac/conventional-type-prompt ()
  (interactive)
  (when use-magit-commit-prompt-p
    (setq use-magit-commit-prompt-p nil)
    (let ((completion-exra-properties '(:annotation-function (lambda (cand) (cdr (assoc cand conventional-commit-type))))))
      (setq type (completing-read "Commit type: " conventional-commit-type nil t))
      (setq scope (read-string "scope: "))
      (message scope)
...
(add-hook 'git-commit-setup-hook 'pac/conventional-type-prompt)
               ...
               :children (
                          ("Work Task"
                           ...
                           :children (("General Task"
                                      ...
                                      ("Work Deadline"
                                       :keys "d"
                                       :timing "DEADLINE: %^{DEADLINE:}t")
                                      ("Work Scheduled Task"
                                       :keys "s"
                                       :timing "SCHEDULED: %^{SCHEDULED:}t")
                                      )
                           )
                          ("Personal Task"
                           :keys "p"
                           :tag "HOME"
                           :children (("Personal General Task"
                                       :keys "t"
                                       :timing "")
                                      ("Personal Deadline"
                                       :keys "d"
