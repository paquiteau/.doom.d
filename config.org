...
#+end_src
* HOTFIX
** evil slow in org mode table
https://github.com/emacs-evil/evil/issues/1623
#+begin_src  emacs-lisp
(advice-remove 'set-window-buffer #'ad-Advice-set-window-buffer)
#+end_src
Adapted from
https://old.reddit.com/r/emacs/comments/x79dds/evil_and_org_tables_what_am_i_missing/inbycsi/
This does not solve the problem completely, as the problem also arise with TAB
#+begin_src emacs-lisp
...
(defun bss/my-org-return ()
  ...
  (cond
   (;; ret in org tables
    (and (equal major-mode 'org-mode) (org-at-table-p)) (call-interactively 'orgtbl-ret))
    ((and (equal major-mode 'org-mode) (org-in-src-block-p)) (org-babel-do-key-sequence-in-edit-buffer (kbd "RET")))
     ...
     (if (evil-normal-state-p)
         (progn ;;we in evil mode
           ...
           (if (and (> 1 (- pos (point) )) need-return)
               (progn (evil-open-above 1)
                      (evil-normal-state)
                      (setq need-return nil)))
           (goto-char pos) (if need-return (org-return)))
       (if (or (eolp) (bolp) ) ;; (newline-and-indent)
           (newline) (org-return))))
...
Lets patch n erd-icons to my taste
#+begin_src emacs-lisp
(after! doom-modeline
(mapcar (lambda (mode)
          (setf (alist-get mode nerd-icons-mode-icon-alist) '(nerd-icons-devicon "nf-dev-git" :face nerd-icons-red)))
        '(magit-popup-mode magit-status-mode magit-log-mode))
(setf (alist-get 'org-mode nerd-icons-mode-icon-alist) '(nerd-icons-mdicon "nf-md-unicorn_variant" :face nerd-icons-lgreen))
)
#+end_src
         ...
         (with-temp-file "org-version.el"
           ...
           (let
             ...
             (insert
              ...
              "(provide 'org-version)\n"))))
  :pin  "8cd4c3944ea41f12b4df2c4fc84dc2fece30ed80")
#+end_src
...
(defcustom lsp-pylsp-plugins-ruff-enabled t
