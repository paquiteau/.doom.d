...
(after! org
  (setq
   ...
   org-cite-biblatex-options "sorting=none, doi=false,url=false,isbn=false,style=ieee"
   org-latex-default-packages-alist '(("AUTO" "inputenc" t ("pdflatex"))
                                    ("T1" "fontenc" t  ("pdflatex"))
                                    ("table,dvipsnames,x11names" "xcolor" nil)
                                    ("colorlinks=true, linkcolor=MidnightBlue,citecolor=OliveGreen,filecolor=magenta,urlcolor=BlueViolet" "hyperref" nil))
...
;; (add-hook 'org-mode-hook '(lambda () (when (string-prefix-p (expand-file-name org-roam-directory) (buffer-file-name))
;;                                        (let ((current-prefix-arg '(16))) ;; magic number in the code.
;;                                          (call-interactively 'org-latex-preview)))))
#+end_src
         ...
         (with-temp-file "org-version.el"
           ...
           (let
             ...
             (insert
              ...
              "(provide 'org-version)\n"))))
  :pin  "382990505b96973791f4fc14f5283d70d3da9183")
#+end_src
...
(after! org-roam
(add-hook 'org-roam-mode-hook 'org-latex-preview-buffer))
#+end_src
...
(defvar pac/copilot-manual-mode nil
  "When t, will only show completion when triggered by M-C-RET")
(defvar pac/no-copilot-mode
  '(shell-mode
...
(use-package! copilot
  ...
  :bind (:map copilot-completion-map
              ...
              ("C-<tab>" . 'copilot-accept-completion-by-word))
  :config
  (add-to-list 'copilot-disable-predicates #'(lambda () (or (member major-mode pac/no-copilot-mode)) (company--active-p))))


(map! :leader :desc "Toggle AI Completion" :n "c p"  (global-copilot-mode))

#+end_src

*** TODO Better integration of copilot
https://robert.kra.hn/posts/2023-02-22-copilot-emacs-setup/
https://github.com/rksm/copilot-emacsd

** Codeium
Codeium is a free (as a beer) alternative to copilot, with direct support for emacs.
...
(use-package! codeium
  ...
  :config
;;  (setq codeium/metadata/api_key (funcall (plist-get (nth 0 (auth-source-search :max 1 :host "codeium.com" :require '(:secret))) :secret)))
...
#+end_src
** Text Modes
There is two main LSP server for plain text editing:
- LSP-LTeX
- Grammarly-ls
...
(use-package! lsp-ltex
  ...
  (setq lsp-ltex-check-frequency "save"
        ...
        lsp-ltex-diagnostic-severity "information" ;; warning, or hint
 ...
 ;; ngrams extensions: check for common mistakes in English (their/ there, etc.)
 ;; language-tools-ngrams package must be installed.
...
(use-package! lsp-grammarly
  :init
  (setq lsp-grammarly-suggestions-passive-voice nil)
)
#+end_src

#+begin_src emacs-lisp

(defvar +lsp-disabled-modes '(org-msg-edit-mode)
  "Major modes where lsp-ltex should be disabled.")

(defun pac/lsp-text--setup (server)
  "Setup lsp client for text."
  (cond ((eq server 'ltex-ls) (require 'lsp-ltex))
        ((eq server 'grammarly-ls) (require 'lsp-grammarly)))
  (when (and (not (memq major-mode +lsp-disabled-modes))
             (not (memq server lsp-disabled-clients))
             (lsp-deferred))))

(defun pac/lsp-text--enabled-p (server)
  "Return t if lsp client is not disable for the current buffer."
  (not (memq server lsp-disabled-clients)))

(defun pac/lsp-text--enable (server)
  "Enable text lsp server for the current buffer."
  (unless (pac/lsp-text--enabled-p server)
    (setq-local lsp-disabled-clients (delq server lsp-disabled-clients))
    (message  (concat "Enabled " (symbol-name server)))
  (pac/lsp-text--setup server)))

(defun pac/lsp-text--disable (server)
  (when (pac/lsp-text--enabled-p server)
    (setq-local lsp-disabled-clients (cons server lsp-disabled-clients))
    (lsp-disconnect)
    (message (concat "Disabled " (symbol-name server)))))

(defun pac/lsp-text-server-toggle (server)
  (interactive (or current-prefix-arg (when (eq lsp-disabled-clients nil) 'grammarly-ls) (list (intern (completing-read "LSP server: " lsp-disabled-clients)))))
  (cond ((eq server 4) (pac/lsp-text--disable 'ltex-ls) (pac/lsp-text--disable 'grammarly-ls))
        ((eq server 'ltex-ls) (pac/lsp-text--enable 'ltex-ls) (pac/lsp-text--disable 'grammarly-ls))
        ((eq server 'grammarly-ls) (pac/lsp-text--enable 'grammarly-ls) (pac/lsp-text--disable 'ltex-ls))
        ))

(map! :map text-mode-map
      :localleader
      :desc "Toggle lsp client for text" :n "G" #'pac/lsp-text-server-toggle)
#+end_src

*** TODO  Customization of the doc of grammarly

#+begin_src  emacs-lisp
;; TODO only run this function when the lsp client is grammarly.
(defun lsp-grammarly-format-span nil
  "find all strings in a span tag and propertize them with a color"
  (interactive)
  (save-excursion
    (goto-char (point-min))
    (while (re-search-forward "\n?\n?<span style=\"color:#F00;\">\\(.*?\\)</span>\n?\\(.*?\\)\n?\u2026" nil t)
      (let* ((len-err (length (match-string 1)))
             (len-cor (length (match-string 2)))
             (err (match-string 1))
             (cor (match-string 2)))
        (set-text-properties 0 len-err (list 'face '(:foreground "#ff6c6b" :weight bold :strike-through t)) err)
        (set-text-properties 0 len-cor (list 'face '(:foreground "#98be65" :weight bold :strike-through nil)) cor)
        (replace-match (concat " " err cor " …"))
        )
      )
    ;; Second pass with simpler regex if the first one did no
    (goto-char (point-min))
    (while (re-search-forward "\n?\n?<span style=\"color:#F00;\">\\(.*?\\)</span>\n?" nil t)
      (let* ((len-err (length (match-string 1)))
             (err (match-string 1)))
...
;;<span style="color:#F00;">test</span>
;;… possibilities for \n<span style=\"color:#F00;\">~~the analysis of~~</span>\nanalyzing …"

(advice-add 'lsp-ui-doc--fill-document :after #'lsp-grammarly-format-span)
#+end_src
#+begin_src emacs-lisp
(after! lsp
...
pylsp is install system-wide, with plugins (notably the ruff one)
