...
#+begin_src emacs-lisp
(defun pac/conventional-type-decorate (type)
  "Decorate the completions candidates with icon prefix and description suffix"
  (let ((type-data (cdr (assoc type conventional-commit-type))))
    (list
     type
     (concat
...
(defun pac/conv-decorate (type)
  (plist-get (cdr (assoc type conventional-commit-type)) :desc)
)

(defun pac/conventional-commit-read-type ()
  (interactive)
  (consult--read conventional-commit-type
                 :prompt "Commit type: "
                 :annotate #'pac/conventional-type-decorate
                 )
)
...
(defun pac/conventional-type-prompt ()
  ...
  (when use-magit-commit-prompt-p
    (setq use-magit-commit-prompt-p nil)
      (setq type (pac/convention-commit-read-type))
...
Python is my main programming language, so let's make it even better.
#+begin_src emacs-lisp :tangle "packages.el"
...
Use the correct sphinx build in environment.
#+begin_src emacs-lisp
(setq! flycheck-rst-sphinx-executable "pyenv exec sphinx-build")
#+end_src
